...
def fixture_repo(path, repository, kind, policy):
 path.mkdir(parents=True); (path/'house-net-control.json').write_text(json.dumps({'repository':repository,'policy_version':policy,'policy_commit':'4673d4bf692b8ac2c55a82306a22fc0fa263c259'}))
 if kind=='knowledge':
...
def env():
 ...
 k=fixture_repo(d/'knowledge','HouseNet-Projects/house-net-knowledge','knowledge','1.4.2'); git_repo(k,'https://github.com/HouseNet-Projects/house-net-knowledge.git')
 v=fixture_repo(d/'vault','HouseNet-Projects/house-net-vault','vault','1.4.2'); git_repo(v,'https://github.com/HouseNet-Projects/house-net-vault.git')
 return d,cp,k,v
