...
class RuntimeIntegrationTests(unittest.TestCase):
    ...
    def test_estate_backed_knowledge_and_vault_synthesis(self):
        ...
        vault=root/'vault'; (vault/'vault').mkdir(); (vault/'vault/references').mkdir(); (vault/'vault/index.json').write_text(json.dumps({'references':[{'id':'V1'}]})); (vault/'vault/references/V1.json').write_text(json.dumps({'id':'V1','purpose':'synthetic','owner_principal':'HOUSE_NET_OWNER','target_system':'test','sensitivity':'RESTRICTED','required_for_bootstrap':False,'last_verified':'2026-09-16'}))
